package com.example.ducvu212.demomvvm.screen.editor.sticker;
...
public abstract class StickerView extends FrameLayout {
    ...
    private float move_orgX = -1;
    private float move_orgY = -1;
            ...
            if (view.getTag().equals("DraggableViewGroup")) {
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        //double statusBarHeight = Math.ceil(25 * getContext().getResources()
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        double angle_diff = Math.abs(Math.atan2(event.getRawY() - scale_orgY,
                                event.getRawX() - scale_orgX) - Math.atan2(scale_orgY - centerY,
                        ...
                        double length2 =
                                getLength(centerX, centerY, event.getRawX(), event.getRawY());

                        int size = convertDpToPixel(SELF_SIZE_DP, getContext());
                        if (length2 > length1 && (angle_diff < 25
                                || Math.abs(angle_diff - 180) < 25)) {
                            //scale up
                        ...
                        double angle =
                                Math.atan2(event.getRawY() - centerY, event.getRawX() - centerX)
                                        * 180 / Math.PI;
    ...
    private void init(Context context) {
        ...
        LayoutParams iv_main_params = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        iv_main_params.setMargins(margin, margin, margin, margin);

        LayoutParams iv_border_params = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.MATCH_PARENT);
        iv_border_params.setMargins(margin, margin, margin, margin);

        LayoutParams iv_scale_params =
                new LayoutParams(convertDpToPixel(BUTTON_SIZE_DP, getContext()),
    ...
    private float[] getRelativePos(float absX, float absY) {
        ...
        float[] pos = new float[] {
                absX - ((View) this.getParent()).getX(), absY - ((View) this.getParent()).getY()
        };
